...
def load_config_vars(filename):
    ...
    FFMPEG Timelimit: ' + str(ffmpeg_timelimit) + ', \
    FFMPEG Preset: ' + str(ffmpeg_preset) + ', \
    FFMPEG Threads: ' + str(ffmpeg_threads) + ', \
    Temp Path: ' + str(temp_path) + ', \
    Telegram Bot Token: ' + str(telegram_token) + '.')
    ...
    FFMPEG TIMELIMIT: ' + str(config["ffmpeg_timelimit"]) + ', \
    FFMPEG Preset: ' str(config["ffmpeg_preset"]) + ', \
    FFMPEG Threads: ' + str(config["ffmpeg_threads"]) + ', \
    Temp Path: ' + str(config["temp_path"]) + ', \
    Telegram Bot Token: ' + str(config["telegram_token"]) + '.')
